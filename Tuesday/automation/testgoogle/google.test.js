...
test("can search Google twice", async () => {
  ...
  await driver.findElement(By.name('q')).sendKeys("Elden Ring", Key.RETURN);
  ...
  await driver.findElement(By.name('q')).sendKeys("Resident Evil Village", Key.RETURN);

  // TODO Wait for the results page to load
  await driver.wait(until.titleIs("Resident Evil Village - Google Search"), 1000);
});
